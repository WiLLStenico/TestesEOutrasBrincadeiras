...
HWND Procura_Tela(char nome_classe[250],char nome_title[250]){
    ...
    while(fcContact==0){
        system("cls");
        printf("\nProcurando Janela %s e %s", nome_classe, nome_title );
        Sleep(time);
        printf(".");
        Sleep(time);
        printf(".");
...
while(i==0){
 EnviaEspecial("WIN",1,0);
 EnviaTecla("d");
 EnviaEspecial("WIN",0,1);

 EnviaEspecial("ENTER",1,1);



Procura_Tela("SALFRAME",NULL);

EnviaEspecial("F2",1,1);

EnviaEspecial("CTRL",1,0);
EnviaTecla("a");
EnviaTecla("c");
EnviaEspecial("CTRL",0,1);
...
EnviaEspecial("ALT",0,1);
*/
Procura_Tela("SunAwtDialog",NULL);
EnviaEspecial("CTRL",1,0);
EnviaTecla("a");
EnviaTecla("v");
EnviaEspecial("CTRL",0,1);
...
//Adicionar um IF se funfar!!
Sleep(2500);
Procura_Tela(NULL,"User ID / Password");
//getch();
/*EnviaEspecial("CTRL",1,0);
EnviaTecla("v");
EnviaEspecial("CTRL",0,1);
...
Procura_Tela(NULL,"SAGRE/CAD");

Procura_Tela(NULL,"Gerenciador de tarefas do Windows");
...
Sleep(15000);
Procura_Tela(NULL,"Sagre/Eng - [Drawing2.dwg]");
